	...
	return (
		<div className="p-6 lg:p-10 space-y-8">
			<div className="flex items-center justify-between">
				<header>
					...
					<h1 className="font-sans text-3xl font-medium tracking-tight">
						Message Layouts — <span className="text-muted-foreground">structured content.</span>
					</h1>
				</header>
				{!showForm && (
					<Button onClick={openCreate} className="font-mono text-[11px] uppercase tracking-wider">
						+ New Template
					</Button>
				)}
			</div>
						...
						<form onSubmit={handleSubmit} className="space-y-6">
							<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
								<div className="space-y-1.5">
									<Label className="font-mono text-[10px] uppercase opacity-60">Template Name</Label>
									<Input
										value={form.name}
										onChange={(e) =>
							...
							{form.type === "email" && (
								<div className="space-y-1.5">
									<Label className="font-mono text-[10px] uppercase opacity-60">Subject Line</Label>
									<Input
										value={form.subject}
										onChange={(e) =>
							...
							<div className="space-y-1.5">
								<Label className="font-mono text-[10px] uppercase opacity-60">Body Content</Label>
								<textarea
							...
							<div className="space-y-1.5">
								<Label className="font-mono text-[10px] uppercase opacity-60">Variables (comma separated)</Label>
								<Input
							...
							<div className="flex gap-3">
								<Button type="submit" className="flex-1 font-mono text-[12px] uppercase tracking-wider">
								...
								<Button
									...
									className="font-mono text-[12px] uppercase tracking-wider px-6"
								>
									Cancel
								</Button>
							</div>
			...
			<div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
				{templates.map((tpl) => (
					<Card key={tpl.id} className="border-hairline bg-muted/5 shadow-none group">
						<CardHeader className="pb-2">
							<div className="flex items-start justify-between">
								<CardTitle className="font-sans text-[15px] font-medium tracking-tight group-hover:text-primary transition-colors">
									{tpl.name}
								</CardTitle>
								<Badge variant={TYPE_VARIANT[tpl.type] ?? "secondary"} className="font-mono text-[9px] h-4 px-1.5 opacity-70">
									{tpl.type}
								</Badge>
							</div>
						</CardHeader>
						<CardContent className="space-y-4">
							{tpl.subject && (
								<p className="text-muted-foreground text-[12px] font-mono line-clamp-1 opacity-70">
									<span className="opacity-50 uppercase text-[10px] mr-1">Sub:</span> {tpl.subject}
								</p>
							)}
							<p className="line-clamp-2 text-[13px] text-muted-foreground opacity-80 h-10">
								{tpl.content}
							</p>
							<div className="flex flex-wrap gap-1">
								{tpl.variables.map((v) => (
									<Badge key={v} variant="outline" className="h-4 font-mono text-[9px] opacity-40 px-1 border-dashed">
										{`{{${v}}}`}
									</Badge>
								))}
							</div>
				...
				{templates.length === 0 && (
					<div className="col-span-full py-20 text-center border-2 border-dashed rounded-lg border-hairline opacity-30">
						<p className="font-mono text-[12px] uppercase">No templates deployed yet.</p>
					</div>
				)}
			</div>
		</div>
