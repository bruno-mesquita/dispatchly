		...
		<Card className="overflow-hidden border-hairline bg-muted/5 shadow-none">
			...
			<CardContent className="p-4 pt-2">
				<div className="flex items-end justify-between gap-4">
					<p className="font-medium font-sans text-2xl tracking-tight">
	...
	return (
		<div className="space-y-8 p-6 lg:p-10">
			<header>
				...
				<h1 className="font-medium font-sans text-4xl tracking-tight">
							...
							<Table>
								<TableHeader className="bg-muted/30">
									<TableRow className="border-none hover:bg-transparent">
										<TableHead className="h-9 px-6 font-mono text-[10px] uppercase tracking-wider">
											Recipient
										</TableHead>
										<TableHead className="h-9 font-mono text-[10px] uppercase tracking-wider">
											Type
										</TableHead>
										<TableHead className="h-9 text-center font-mono text-[10px] uppercase tracking-wider">
											Status
										</TableHead>
										<TableHead className="h-9 px-6 text-right font-mono text-[10px] uppercase tracking-wider">
											Timestamp
										</TableHead>
									</TableRow>
								</TableHeader>
								<TableBody>
									{recentLogs.map((log) => (
										<TableRow key={log.id} className="group border-muted/10">
											<TableCell className="max-w-40 truncate px-6 font-mono text-[12px] opacity-80 transition-opacity group-hover:opacity-100">
												{log.to}
											</TableCell>
											<TableCell className="font-mono text-[10px] uppercase tracking-wider opacity-60">
												{log.type}
											</TableCell>
											<TableCell className="text-center">
												<Badge
													variant={STATUS_VARIANT[log.status] ?? "secondary"}
													className="h-5 px-1.5 font-mono text-[9px] uppercase tracking-tight"
												>
													{log.status}
												</Badge>
											</TableCell>
							...
							{[
								...
								>
									<div className="space-y-0.5">
										<p className="font-medium font-sans text-[14px]">
											{action.label}
										</p>
					...
					<div className="space-y-4">
						...
						<div className="grid grid-cols-3 gap-2">
							<div className="rounded-md border border-hairline bg-muted/5 p-3 text-center">
								...
								<p className="font-medium font-sans text-lg">{byType.email}</p>
							</div>
							<div className="rounded-md border border-hairline bg-muted/5 p-3 text-center">
								<p className="mb-1 font-mono text-[10px] text-muted-foreground uppercase">
									SMS
								</p>
								<p className="font-medium font-sans text-lg">{byType.sms}</p>
							</div>
							<div className="rounded-md border border-hairline bg-muted/5 p-3 text-center">
								<p className="mb-1 font-mono text-[10px] text-muted-foreground uppercase">
									Push
								</p>
								<p className="font-medium font-sans text-lg">{byType.push}</p>
							</div>
						</div>
					</div>
				</div>
