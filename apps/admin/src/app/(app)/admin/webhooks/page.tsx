	...
	return (
		<div className="flex flex-col gap-6">
			...
			<div className="grid grid-cols-4 gap-px overflow-hidden rounded-md border bg-border">
				{[
					{ label: "Active Hooks", value: webhooks.filter(w => w.isActive).length },
						...
						<Table>
							<TableHeader className="bg-muted/20">
								<TableRow className="hover:bg-transparent border-b">
									<TableHead className="h-9 font-mono text-[10px] uppercase">Name</TableHead>
									<TableHead className="h-9 font-mono text-[10px] uppercase">Organization</TableHead>
									<TableHead className="h-9 font-mono text-[10px] uppercase">URL</TableHead>
									<TableHead className="h-9 font-mono text-[10px] uppercase text-center">Status</TableHead>
									<TableHead className="h-9 font-mono text-[10px] uppercase text-right">Last Triggered</TableHead>
								</TableRow>
							</TableHeader>
							<TableBody>
								{webhooks.map((wh) => (
									<TableRow key={wh.id} className="group border-b last:border-0 hover:bg-muted/30 transition-colors">
										...
										<TableCell className="py-3">
											<code className="text-[11px] text-accent/80 bg-accent-tint px-1.5 py-0.5 rounded border border-accent/10 truncate block max-w-xs">
												{wh.url}
											</code>
										</TableCell>
										<TableCell className="py-3">
											<div className="flex justify-center">
												<div className={cn(
													"flex items-center gap-2 px-2 py-0.5 rounded-full border text-[10px] font-bold uppercase tracking-wider",
													wh.isActive 
														? "bg-green-500/10 border-green-500/20 text-green-600 dark:text-green-400" 
														: "bg-muted border-border text-muted-foreground"
												)}>
													{wh.isActive && <PulseDot size={4} color="currentColor" />}
										...
										<TableCell className="py-3 text-right font-mono text-[11px] text-muted-foreground">
											{wh.lastTriggeredAt
												? new Date(wh.lastTriggeredAt).toLocaleString([], { hour: '2-digit', minute: '2-digit', second: '2-digit', day: '2-digit', month: '2-digit' })
												: "---"}
										</TableCell>
									</TableRow>
								))}
