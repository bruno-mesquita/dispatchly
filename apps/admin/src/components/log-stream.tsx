...
export function LogStream({ rows = 20, speed = 1200, className = "" }) {
	...
	return (
		<div
			className={cn(
				"font-mono text-[11px] text-code-fg leading-relaxed",
				className,
			)}
		>
			{log.map((r) => (
				<div
					key={r.id}
					className="fade-in slide-in-from-bottom-1 flex animate-in gap-3 duration-300"
				>
					...
					<span
						className={cn(
							"w-6 shrink-0 font-bold uppercase",
							lvlColor[r.lvl as keyof typeof lvlColor],
						)}
					>
						{r.lvl.toUpperCase().slice(0, 3)}
					</span>
					<span className="w-10 shrink-0 text-muted-foreground/60 uppercase tracking-tighter">
						{r.ch}
					</span>
