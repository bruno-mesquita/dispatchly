...
// Mock dependencies
const mockCheckQuota = mock(() =>
	Promise.resolve({ allowed: true, remaining: 100 }),
);
const mockIncrementUsage = mock(() => Promise.resolve());
const mockAddToQueue = mock(() => Promise.resolve());
const mockApplyTemplate = mock((id, vars) =>
	Promise.resolve({ subject: "Rendered Subject", content: "Rendered Content" }),
);

mock.module("@dispatchly/billing", () => ({
	checkQuota: mockCheckQuota,
	incrementUsage: mockIncrementUsage,
	PLANS: [{ id: "free", limits: { emails: 100, sms: 50, push: 100 } }],
}));
...
describe("Notifications Router", () => {
	...
	it("sends a notification successfully and increments usage", async () => {
		...
		expect(mockIncrementUsage).toHaveBeenCalledWith("org_123", "emails");
	});
	...
	it("uses templates successfully", async () => {
		const caller = notificationsRouter.createCaller(mockCtx as any);
		...
		expect(mockApplyTemplate).toHaveBeenCalledWith("tmpl_1", { name: "Alice" });
	});

	it("fails when template variables are missing", async () => {
		mockApplyTemplate.mockImplementationOnce(() =>
			Promise.resolve({
				subject: "Hello {{name}}",
		...
		try {
			...
			expect(e.message).toContain("Missing variables");
		}
	});
});
